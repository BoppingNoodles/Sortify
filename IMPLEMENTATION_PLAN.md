...
| Phase | Weeks | Goal |
|---|---|---|
| 🟢 **Onboarding & Setup** | 1 | Dev tools installed, Figma wireframing started, core tool mini-exercises completed |
...
> **Theme:** Get everyone on the same page. Install tools, learn the stack, design the app flow in Figma, and complete a hands-on mini-exercise.

## High-Level Goals
- [ ] All members have their dev environment fully set up
- [ ] Frontend team sets up Figma (Education Plan) and drafts low-fidelity wireframes for the core app flow
...
### 📱 Frontend (React Native + Expo & Figma UI/UX)

> [!IMPORTANT]
> **Design-First Principle:** Do not write production UI code until the app is wireframed and designed in Figma! Designing first ensures the team agrees on the exact user flow, button placements, color schemes, and component hierarchy before spending hours writing React Native styles.

**Setup:**
- Sign up for a free **Figma Education Account** at [figma.com/education](https://www.figma.com/education/) using your `@berkeley.edu` email (gives free Pro collaboration features)
- Install Node.js (LTS) and VS Code
- Install the **Expo Go** app on your personal phone (iOS App Store or Google Play Store) — this is how you'll test mobile code
- Clone the repo and verify Expo setup: `npx create-expo-app@latest sortify-app` and test `npx expo start`

**Primary Task — Figma Wireframing & App Flow:**
> Before writing production code, the frontend team will collaborate on a shared Figma project to map out the user experience:
> 1. **User Flow Mapping:** Map the complete user journey: Welcome/Onboarding → Camera Scan Screen → Classification Result Modal → Habits/Streak Profile → Local Rules Guide.
> 2. **Low-Fidelity Screen Wireframes (Mobile Frames):**
>    - **Camera Scan Screen:** Fullscreen viewfinder, flash toggle, shutter button, gallery upload button.
>    - **Result Modal / Screen:** Big bold classification badge (Green for Compost, Blue for Recycling, Gray/Black for Landfill), detected item name, AI confidence score, and clear disposal instructions (e.g., "Rinse container first").
>    - **Engagement / Profile Screen:** Daily sorting streak counter, total items scanned, badge progress.
>    - **Rules / Search Screen:** Quick search list for campus items and disposal guidelines.
> 3. **Deliverable:** Share the Figma link in the team workspace for Caden, Arnav, and the sub-teams to review and give feedback.

**Code Learning Mini-Exercise:**
> Build a minimal sandbox "Camera Capture" screen in Expo (to verify mobile hardware permissions and Expo Go):
> 1. Uses `expo-camera` to display a live camera viewfinder on your physical phone
...
>
> **Why:** Verifies camera hardware access and Expo Go workflows on actual phones while designs are being finalized.

**Resources to study:**
- [Figma for Beginners (Official Tutorial)](https://www.youtube.com/playlist?list=PLXDU_eVOJTx7QHLShNqIXL1Cgbxj7HlN4) — 30 minutes to learn frames, auto-layout, and components
...
- [ ] **Draw the system architecture diagram** — use draw.io or Excalidraw:
  ...
  ```
- [ ] Configure the Firebase project at the [Firebase Console (console.firebase.google.com)](https://console.firebase.google.com/) — enable **Authentication** (enable the Email/Password sign-in provider) and create a **Cloud Firestore** database in test mode
